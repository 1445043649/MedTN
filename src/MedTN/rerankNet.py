...
class MyData(Dataset):
    def __init__(self, ids, type_ids, mask):
        super(MyData, self).__init__()
        self.input_ids = ids.tolist()
        self.attention_mask = mask.tolist()
        self.token_type_ids = type_ids.tolist()
...
def collate_fn(term):
    ...
    for i in term:
        ...
        type_ids.append(i[1])
        attention_mask.append(i[2])
    ids, token_type_ids, mask = torch.tensor(input_ids).long(), torch.tensor(type_ids).long(), torch.tensor(attention_mask).long()
    return ids.to('cuda'), token_type_ids.to('cuda'), mask.to('cuda')


class RerankNet(nn.Module):
    def __init__(self, encoder, learning_rate,sparse_learning_rate, weight_decay, ori_sparse_weight, aug_sparse_weight, use_cuda):

        LOGGER.info("RerankNet! learning_rate={} sparse_learning_rate={} weight_decay={} ori_sparse_weight={} aug_sparse_weight={} use_cuda={}".format(
            learning_rate,sparse_learning_rate,weight_decay,ori_sparse_weight,aug_sparse_weight,use_cuda
        ))
        ...
        self.optimizer = optim.Adam([
            ...
            {'params' : self.ori_sparse_weight, 'lr': self.sparse_learning_rate, 'weight_decay': 0},
            {'params' : self.aug_sparse_weight, 'lr': self.sparse_learning_rate, 'weight_decay': 0}], 
    ...
    def forward(self, x):
        ...
        output : (N, topk)
        """
        query_token, candidate_tokens, ori_candidate_s_scores, aug_candidate_s_scores = x
        batch_size, topk, max_length = candidate_tokens['input_ids'].shape

        if self.use_cuda:
            ori_candidate_s_scores = ori_candidate_s_scores.cuda()
            aug_candidate_s_scores = aug_candidate_s_scores.cuda()
        ...
        score = self.aug_sparse_weight * aug_candidate_s_scores + self.ori_sparse_weight * ori_candidate_s_scores + candidate_d_score
        return score
